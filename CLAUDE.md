...
| `workspace/scratch/` | **Ephemeral thinking.** Write a script, run it, read it, delete it. Gitignored; nothing here is durable. |
| `workspace/notes/journal.md` | **Persistent memory.** Read it at session start; append what was learned before a substantial task ends. |
| `workspace/proposals/` | Proposed edits to `docs/` or `CLAUDE.md`, as diffs. You cannot write those files directly. |
| `workspace/exports/` | Generated deliverables — xlsx, png, csv. Written through `agent/exports.py`, which confines every write here. |
| `workspace/runs/` | Immutable run manifests + outputs. Record one for any figure you report; read them from `index.jsonl` or `agent.lineage.list_runs()`. |
...
| `pipeline/` | `config.py`, `queries.py` (**registry — adding one needs human review**), `pull.py`, `checks.py` (**run before reporting**), and the CLIs: `targets_cli`, `waterfall_cli`, `slip_cli`, `export_cli` — all runnable as `python -m pipeline.X` or importable. |
...
**Use `python -m pipeline.export_cli excel|chart`, or import `agent.exports`.**
`export_path()` is the write boundary — supply a NAME, never a path, so no
filename (including one that arrived in a prompt) can write outside
`workspace/exports/`. Do not hand-roll a file write.

**Scratch scripts are ephemeral — delete them when the task ends.** A script
worth keeping is not scratch: move it into `pipeline/` with a test.
...
- Compute in `workspace/scratch/`; import `pipeline/` modules rather than
  re-deriving their logic. Run `pipeline.checks.run_all` on any output before
  reporting a figure from it, and say which layer backed the number.